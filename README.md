...
[![Language](https://img.shields.io/badge/Lang-C%20%2B%20ASM%20%2B%20C++-orange)]()
...
- **Gerenciamento Avançado de Memória:** Implementação de uma `KernelHeap` robusta em C++ OOP. Apresenta algoritmo *First-Fit* para alocação e suporte a *Coalescing* automático (fusão de blocos adjacentes vazios) na liberação real de memória. Sobrecarga global dos operadores nativos `new`, `new[]`, `delete` e `delete[]`.
...
- **Gerenciador de Janelas (WM):** Organizado sob uma hierarquia de classes orientada a objetos em C++ (`Widget`, `Window2`, `Button`, etc.). Suporta foco, arraste responsivo pelo título e fechamento de janelas via mouse.
...
├── linker.ld       # Script de ligação para organização da memória (Pronto para C++17)
...
```bash
sudo apt install gcc-x86-64-linux-gnu g++-x86-64-linux-gnu nasm grub-pc-bin xorriso qemu-system-x86
...
2. **Escalabilidade do FS:** O VFS dinâmico na RAM ainda possui limite máximo de 32 nós (arquivos ou diretórios).
3. **Capacidade de Arquivo:** O tamanho máximo por arquivo individual está fixado em 256 bytes.
4. **Isolamento de Processos:** O sistema opera integralmente em Ring 0 (Kernel Mode) sem separação de espaço de usuário e multitarefa preemptiva.
